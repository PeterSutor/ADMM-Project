...
% DESCRIPTION -------------------------------------------------------------
%
% Tests the Least Absolute Deviations (LAD) problem with ADMM. LAD
...
% of minimizing function obj(x,z) under the constraint. See function lad
...
%
% NOTE: If ladtest is executed with no arguments, it will create a demo
% test of random data of size m = 2^9, n = 2^6. The random values are
...
function [results, test] = ...
    ladtest(seed, rows, cols, errtol, quiet, options)
% INPUTS ------------------------------------------------------------------
...
% Create a random problem for Least Absolute Deviations.
D = randn(rows, cols);          % Create a normally distributed random D.
xtrue = 10*randn(cols, 1);      % Make a random scalar multiple as true x.
s = D*xtrue;                    % Make a soon-to-be noisy signal s.

% Add random, largely deviating, noise to our noise signal, at random
% indices in signal s.
indices = randsample(rows, ceil(rows/50));
s(indices) = s(indices) + 100*randn(size(indices));

% Our original x value and associated objective value.
truexopt = xtrue;
trueobjopt = norm(D*xtrue - s, 1);
...
objopt = norm(D*xopt - s, 1);       % The actual objective value computed.

% Compute the relative and componentwise average error in x values from 
% true solution.
xresidual = norm(truexopt - xopt, 'fro');
xerror = sum(abs(truexopt - xopt))/length(xopt);

% Check to see if the test was successful or not.
if (xresidual < errtol && abs(objopt - trueobjopt) <= errtol*trueobjopt)
    failed = 0;
    failreason = ['The objective value is within a relative error of ', ...
        num2str(errtol), ' and so is the norm of the difference ', ...
        ' between the true and recovered signal x.'];
elseif xresidual < errtol 
    failed = 1;
    failreason = ['The norm ||x_true - x_recovered|| is not smaller', ...
        ' than the specified error tolerance ', num2str(errtol)];
elseif abs(objopt - trueobjopt) <= errtol*trueobjopt
    failed = 1;
    failreason = ['The objective value from the recovered x is not', ...
        ' within a relative error tolerance of ', num2str(errtol), ...
        ' to the true signal x.'];
else
    failed = 1;
    failreason = ['The norm ||x_true - x_recovered|| and relative', ...
        ' objective error from the true and recovered signal x', ...
        ' is not smaller than ', num2str(errtol)];
end
...
function [seed, rows, cols, errtol, quiet] = ...
    errorchecker(seed, rows, cols, errtol, quiet, options)
% INPUTS ------------------------------------------------------------------
% Same as for the ladtest function.
% 
% OUTPUTS -----------------------------------------------------------------
